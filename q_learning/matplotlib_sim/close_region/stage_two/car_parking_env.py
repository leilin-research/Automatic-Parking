...
class car_sim_env(object):
    ...
    def __init__(self):
        ...
        self.wall_verts = self.get_rect_verts(self.wall_center, 3.5, 2.3, angle=0.0)
        ...
        self.ax.add_patch(self.terminal1_patch)
        self.ax.add_patch(self.terminal2_patch)
        self.ax.add_patch(self.terminal3_patch)
        self.ax.add_patch(self.terminal4_patch)
        self.ax.add_patch(self.terminal5_patch)
    ...
    def get_terminal_pose(self):
        ...
        self.terminal_boundary = np.zeros((5,4))
        self.terminal_xy = np.zeros((5,2))

        self.terminal_xy[0][0] = 0.8
        self.terminal_xy[0][1] = -1.4

        self.terminal_xy[1][0] = -0.8
        self.terminal_xy[1][1] = -1.4

        self.terminal_xy[2][0] = -0.8
        self.terminal_xy[2][1] = -2.6

        self.terminal_xy[3][0] = 0.8
        self.terminal_xy[3][1] = -2.6

        self.terminal_xy[4][0] = 0.0
        self.terminal_xy[4][1] = -2.0

        for i in range(5):
            self.terminal_boundary[i][0] = self.terminal_xy[i][0] - x_offset
            self.terminal_boundary[i][1] = self.terminal_xy[i][0] + x_offset
            self.terminal_boundary[i][2] = self.terminal_xy[i][1] - y_offset
            self.terminal_boundary[i][3] = self.terminal_xy[i][1] + y_offset


        self.terminal1_patch = plt.Circle(self.terminal_xy[0, :], 0.06, color='green')
        self.terminal2_patch = plt.Circle(self.terminal_xy[1, :], 0.06, color='green')
        self.terminal3_patch = plt.Circle(self.terminal_xy[2, :], 0.06, color='green')
        self.terminal4_patch = plt.Circle(self.terminal_xy[3, :], 0.06, color='green')
        self.terminal5_patch = plt.Circle(self.terminal_xy[4, :], 0.06, color='green')
    ...
    def reach_terminal(self, pose):
        ...
        if pose[0] >= self.terminal_boundary[0][0] + x_offset and pose[0] <= self.terminal_boundary[0][1] - x_offset \
            and pose[1] > self.terminal_boundary[0][2] + y_offset and pose[1] < self.terminal_boundary[0][3] - y_offset \
            and (pose[2] == 0):
            return True

        elif pose[0] >= self.terminal_boundary[1][0] + x_offset and pose[0] <= self.terminal_boundary[1][1] - x_offset \
            and pose[1] >= self.terminal_boundary[1][2] + y_offset and pose[1] <= self.terminal_boundary[1][3] - y_offset \
            and (pose[2] == 8):
            return True

        elif pose[0] >= self.terminal_boundary[2][0] + x_offset and pose[0] <= self.terminal_boundary[2][1] - x_offset \
            and pose[1] >= self.terminal_boundary[2][2] + y_offset and pose[1] <= self.terminal_boundary[2][3] - y_offset \
            and (pose[2] == 8):
            return True

        elif pose[0] >= self.terminal_boundary[3][0] + x_offset and pose[0] <= self.terminal_boundary[3][1] - x_offset \
            and pose[1] >= self.terminal_boundary[3][2] + y_offset and pose[1] <= self.terminal_boundary[3][3] - y_offset \
            and (pose[2] == 0):
            return True

        elif pose[0] >= self.terminal_boundary[4][0] + x_offset and pose[0] <= self.terminal_boundary[4][1] - x_offset \
            and pose[1] >= self.terminal_boundary[4][2] + y_offset and pose[1] <= self.terminal_boundary[4][3] - y_offset \
    ...
    def cal_deadline(self, x, y):
        dist = np.zeros(5)
        for i in range(5):
            dist[i] = abs(x - self.terminal_xy[i][0]) / self.grid_width + abs(y - self.terminal_xy[i][1]) / self.grid_width
        deadline = max(int(min(dist)) * 6,20)
        deadline = min(deadline, 60)
        return deadline
