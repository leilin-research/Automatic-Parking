...
class LearningAgent(Agent):
    ...
    def init_q_tables(self, path):
        filelists = os.listdir(path)
        for file in filelists:
            with open(os.path.join(path, file), 'rb') as f:
                self.Q_values.update(cPickle.load(f))
...
def train(env, agt, restore, LfD = False):
    ...
    for trial in xrange(max_index + 1, n_trials):
        ...
        if not agt.test:
            if trial > 200000 and trial < 500000:
                agt.epsilon = 0.3
            elif trial > 500000 and trial < 800000:
                agt.epsilon = 0.2
            elif trial > 800000:
                agt.epsilon = 3200 / float(trial)
