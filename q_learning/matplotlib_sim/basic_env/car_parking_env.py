...
class car_sim_env(object):
    ...
    valid_actions_dict = {valid_actions[0]: np.array([0.0, 0.0]), \
                          ...
                          valid_actions[6]: np.array([-speed, -angle])}
    def __init__(self):
        self.done = False
        ...
        self.wall_center = np.array([0,-2.5])
        self.wall_edge_length = 8.0
        self.wall_verts = self.get_rect_verts(self.wall_center, 3, 1.5, angle=0.0)
    ...
    def sense(self):
        ...
        agent_pose[0] = int(agent_pose[0] / self.grid_width) * self.grid_width + 0.5 * self.grid_width
        agent_pose[1] = int(agent_pose[1] / self.grid_width) * self.grid_width + 0.5 * self.grid_width
    ...
    def reach_terminal(self, pose):
        ...
        if pose[0] > self.terminal_boundary[0] + x_offset and pose[0] < self.terminal_boundary[1] - x_offset \
            ...
            and (pose[2] == 0 or pose[2] == 8 or pose[2] == 1 or pose[2] == 7 or pose[2] == 9 or pose[2] == 15):
            return True
        return False
