...
class car_sim_env(object):
    ...
    def __init__(self):
        ...
        self.terminal_center = np.array([0, -2.0])
        self.terminal_verts = self.get_rect_verts(self.terminal_center, 3.5, 2.3, angle=0.0)
        print 'terminal_verts',self.terminal_verts
        self.terminal_verts_closed = self.close_rect(self.terminal_verts)
        ...
        self.terminal_path = Path(self.terminal_verts_closed, self.rect_codes)
        ...
        self.terminal_patch = mpl_patches.PathPatch(self.terminal_path, edgecolor='green',facecolor='none', alpha=1,lw=1)
    ...
    def get_terminal_pose(self):
        x_offset = 0.049 * self.grid_width
        y_offset = 0.049 * self.grid_width
        self.terminal_boundary = np.zeros(4)
        self.terminal_xy = np.zeros(2)
        self.terminal_xy[0] = 0
        self.terminal_xy[1] = -2.0

        self.terminal_boundary[0] = self.terminal_verts[0, 0] + 1 * x_offset
        self.terminal_boundary[1] = self.terminal_verts[1, 0] - 1 * x_offset
        self.terminal_boundary[2] = self.terminal_verts[-1, 1] + 1 * y_offset
        self.terminal_boundary[3] = self.terminal_verts[0, 1] - 1 * y_offset
    ...
    def sense(self):
        ...
        if agent_pose[0] > self.terminal_verts[0, 0] and agent_pose[0] < self.terminal_verts[1, 0] \
            and agent_pose[1] > self.terminal_verts[-1, 1] and agent_pose[1] < self.terminal_verts[0, 1]:
            grid_width = 0.1
    ...
    def collide_terminal_with_pose(self, pose):
        verts = self.get_rect_verts(pose[:2], self.car_length, self.car_width, pose[2])
        terminal_collision = tools.two_rects_intersect(verts, self.terminal_verts)

        return terminal_collision
    ...
    def generate_agent_pose(self):
        ...
        while True:
            ...
            if x < self.terminal_verts[1,0] and x > self.terminal_verts[0,0] \
                and y < self.terminal_verts[0,1] and y > self.terminal_verts[-1,1]:
                continue
            elif self.collide_walls_with_pose(np.array([x, y ,theta])):
                continue
            elif self.collide_terminal_with_pose(np.array([x, y ,theta])):
                continue
            else:
                break
