...
class LearningAgent(Agent):
    ...
    def q_table_initialization(self, data_path):
        init_times = 40000
        tmp_Q_values = {}
        for i in range(init_times):
            x,y,z,theta = self.q_gen_agent_pose()
            agent_pose = np.array([x,y,theta])
            agent_pose[2] = (agent_pose[2] + 2 * np.pi) % (2 * np.pi)
            agent_pose[0] = int(agent_pose[0] / self.env.grid_width) * self.env.grid_width + 0.5 * self.env.grid_width
            agent_pose[1] = int(agent_pose[1] / self.env.grid_width) * self.env.grid_width + 0.5 * self.env.grid_width
            idx = np.floor(agent_pose[2] / (self.env.angle_blockwidth / 2))
            if idx % 2 == 0:
                idx = idx / 2
            else:
                idx = (idx + 1) / 2
            ...
            ideal_action = self.env.ideal_action_towards_terminal(agent_pose)
            if ideal_action != '':
                state = states(x=agent_pose[0], y=agent_pose[1], theta=agent_pose[2])
                self.Q_values[(state, ideal_action)] = 2.0
                tmp_Q_values[(state, ideal_action)] = 2.0
        with open(os.path.join(data_path, 'demo_q' + '.cpickle'), 'wb') as f:
            cPickle.dump(tmp_Q_values, f, protocol=cPickle.HIGHEST_PROTOCOL)
    ...
    def q_gen_agent_pose(self):
        while True:
            x = random.uniform(self.env.agent_move_boundary_ini_q[0][0], self.env.agent_move_boundary_ini_q[0][1])
            y = random.uniform(self.env.agent_move_boundary_ini_q[0][2], self.env.agent_move_boundary_ini_q[0][3])
            z = 0.105
            # theta = 0
            theta = random.uniform(0, 2 * np.pi)
            if not self.q_collision_check(x, y):
                break
        return [x, y, z, theta]

    def q_collision_check(self, x, y):
        if x < self.env.agent_move_boundary_ini_q[0][0] or x > self.env.agent_move_boundary_ini_q[0][1] \
                or y < self.env.agent_move_boundary_ini_q[0][2] or y > self.env.agent_move_boundary_ini_q[0][3]:
            return True

        elif x > self.env.agent_move_boundary_ini_q[1][0] and x < self.env.agent_move_boundary_ini_q[1][1] \
                and y > self.env.agent_move_boundary_ini_q[1][2] and y < self.env.agent_move_boundary_ini_q[1][3]:
            return True
        return False
