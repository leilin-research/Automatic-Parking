...
class Environment(object):
    ...
    def act(self, agent, action):
        ...
        # else:
        #     ideal_act = self.ideal_action_towards_terminal(agent_pose)
        #     if ideal_act != '' and action == ideal_act:
        #         reward = 3.0
        #     elif ideal_act != '':
        #         reward = -3.0
    ...
    def ideal_action_towards_terminal(self,agent_pose):
        car_theta = agent_pose[2] * self.angle_blockwidth
        delta_x = self.terminal_xy[0] - agent_pose[0]
        delta_y = self.terminal_xy[1] - agent_pose[1]

        ideal_action = ''
        if abs(delta_x) > 2 * self.car_length or abs(delta_y) > 1.5 * self.car_width:
            car_to_terminal_angle = (np.arctan2(delta_y, delta_x) + 2 * np.pi) % (2 * np.pi)
            target_direction = (car_theta - car_to_terminal_angle + 2 * np.pi) % (2 * np.pi)
            if target_direction > np.pi:
                target_direction -= 2 * np.pi
            if target_direction >= -np.pi / 4 and target_direction <= np.pi / 4:
                ideal_action = 'forward'
            elif target_direction > np.pi / 4 and target_direction <= np.pi / 2:
                ideal_action = 'right_45_forward'
            elif target_direction >= -np.pi / 2 and target_direction < -np.pi / 4:
                ideal_action = 'left_45_forward'
            elif target_direction > np.pi / 2 and target_direction < np.pi * 3 / 4:
                ideal_action = 'right_45_backward'
            elif target_direction >= np.pi * 3 / 4 and target_direction <= np.pi:
                ideal_action = 'backward'
            elif target_direction >= -np.pi and target_direction <= -np.pi * 3 /4:
                ideal_action = 'backward'
            elif target_direction > -np.pi * 3 / 4 and target_direction < -np.pi / 2:
                ideal_action = 'left_45_backward'
    ...
    def get_terminal_pose(self):
        # x_offset = 0 # 0.52 * self.car_length
        # y_offset = 0 # 0.05 * self.car_width
        # self.terminal_boundary = np.zeros(4)
        # self.terminal_xy = np.zeros(2)
        # self.terminal_boundary[0] = self.cars_boundary[1][1] + x_offset
        # self.terminal_boundary[1] = self.cars_boundary[0][0] - x_offset
        # self.terminal_boundary[2] = self.cars_boundary[0][2] + y_offset
        # self.terminal_boundary[3] = self.cars_boundary[0][3] - y_offset
        # self.terminal_xy[0] = (self.cars_boundary[1][1] + self.cars_boundary[0][0]) / 2.0
        # self.terminal_xy[1] = (self.cars_boundary[0][2] + self.cars_boundary[0][3]) / 2.0
    ...
    def reach_terminal(self, pose):
        # angle_tol = 0.174   # 10 degree
        # angle_tol = 0.0873
        angle_tol = 0.348
        # if pose[0] > self.terminal_boundary[0] and pose[0] < self.terminal_boundary[1] and \
        #     pose[1] > self.terminal_boundary[2] and pose[1] < self.terminal_boundary[3] and \
        #         (pose[2] == 0 or pose[2] == 8 or pose[2] == 1 or pose[2] == 7 or pose[2] == 9 or pose[2] == 15):
        #     print 'pose:',pose
        #     print 'terminal_boundary:',self.terminal_boundary
        #     return True
        # else:
        #     return False
    ...
    def set_agent_appro_move_boundary(self):
        ...
        near_fixed_car_tolerance = 1 * self.car_length

        x_offset = 0.55 * self.car_length
        y_offset = 0.8 * self.car_width
        self.agent_move_boundary = np.zeros((2, 4))  # xmin, xmax, ymin, ymax (Outer Suqare, Innder boundary square)

        # self.agent_move_boundary[0][0] = self.cars_boundary[0][0] + 0.6 * self.car_length
        # self.agent_move_boundary[0][1] = self.cars_boundary[0][1] - 0.2 * self.car_length
        # self.agent_move_boundary[0][2] = self.cars_boundary[0][3] + y_offset
        # self.agent_move_boundary[0][3] = self.cars_boundary[0][3] + 1.1 * y_offset

        self.agent_move_boundary[0][0] = -0.8#-0.84
        self.agent_move_boundary[0][1] = -0.8#-0.76
        self.agent_move_boundary[0][2] = -1.4#-1.44
        self.agent_move_boundary[0][3] = -1.4#-1.36

        # self.agent_move_boundary[1][0] = self.cars_boundary[1][0] - near_fixed_car_tolerance
        # self.agent_move_boundary[1][1] = self.cars_boundary[0][1] + near_fixed_car_tolerance
        # self.agent_move_boundary[1][2] = self.cars_boundary[0][2] - near_fixed_car_tolerance
        # self.agent_move_boundary[1][3] = self.cars_boundary[0][3] + near_fixed_car_tolerance

        self.agent_move_boundary[1][0] = 0
        self.agent_move_boundary[1][1] = 0
        self.agent_move_boundary[1][2] = 0
        self.agent_move_boundary[1][3] = 0
        # print "self.agent_move:",self.agent_move_boundary
